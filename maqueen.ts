...
namespace maqueen {
    ...
    /**
     * Run a motor in a specified direction at a given speed.
     */
    //% block="run %motor in %dir at speed %speed"
    //% group="Movement"
    //% weight=90
    //% speed.min=0 speed.max=255
    export function motor_run(motor: MotorPort, dir: Direction, speed: number): void {
        let buf = pins.createBuffer(3);

        if (motor == MotorPort.leftMotor) {
            buf[0] = 0x00;
            buf[1] = dir;
            buf[2] = speed;
            pins.i2cWriteBuffer(0x10, buf);
        } else if (motor == MotorPort.rightMotor) {
            buf[0] = 0x02;
            buf[1] = dir;
            buf[2] = speed;
            pins.i2cWriteBuffer(0x10, buf);
